...
export default function HouseholdPanel({ household, currentUserId, location, onSaveLocation, onLeave, onDelete }) {
  ...
  }, [location.latitude, location.longitude, location.pincode]);

  const isCreator = household.createdById === currentUserId;
  ...
  return (
    <section className="household-panel">
      <button className="link" onClick={() => setOpen((v) => !v)}>
        {open ? 'Hide household settings' : `Household · ${household.members.length} member${household.members.length === 1 ? '' : 's'}`}
      </button>
      ...
      {open && (
        <div className="household-card">
            ...
            {household.members.map((m) => (
              <li key={m.id} className="member-row">
                <span className="member-name">
                  {m.name}
                  {m.id === currentUserId && <span className="member-tag">you</span>}
          ...
          <p className="muted small">
            Share this code — they enter it under &ldquo;Join a household&rdquo;.
          </p>
          <div className="invite-row">
            <code className="invite-code-box">{household.inviteCode}</code>
            <button type="button" onClick={() => navigator.clipboard?.writeText(household.inviteCode)}>Copy</button>
          </div>
          ...
          {showDanger ? (
            <div className="danger-zone">
              <p className="muted small">
                Leaving removes you from the shared list. {isCreator
                  ? 'Since you created it, it passes to whoever joined next.'
                  : 'Everyone else keeps it.'}
              </p>
              <button type="button" className="danger-btn" disabled={busy} onClick={handleLeave}>
                Leave household
              </button>
